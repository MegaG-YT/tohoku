...
export function AssetValue() {
  ...
  const fadeUp = (delay = 0) => ({
    ...
    transition: prefersReducedMotion ? { duration: 0 } : { duration: 0.6, ease: "easeOut" as const, delay },
  })
  ...
  return (
    <section id="asset-value" className="scroll-mt-32 overflow-hidden" ref={ref}>

      {/* Image + headings — mobile: aspect-ratio box; desktop: full overlay */}
      <div className="relative aspect-[4/3] md:aspect-auto md:min-h-[780px]">
        <Image
          src="/images/asset-value-map.png"
          alt="福島市泉エリアの航空写真と立地適正化計画の区域図"
          width={3840}
          height={3597}
          className="absolute inset-0 w-full h-full object-cover scale-105 origin-top"
        />
        {disclaimer}

        {/* Headings overlaid on image on both breakpoints */}
        <div className="absolute inset-x-0 top-0 z-10 pt-10 md:pt-20 lg:pt-24 px-4">
          <div className="mx-auto max-w-7xl text-center">
            {headings}
          </div>
        </div>

        {/* Card overlaid — desktop only */}
        <div className="hidden md:block absolute inset-x-0 top-0 z-10 pt-20 lg:pt-24">
          <div className="mx-auto max-w-7xl px-4 mt-40 lg:mt-48">
            {card}
          </div>
        </div>
      </div>

      {/* Card below image — mobile only */}
